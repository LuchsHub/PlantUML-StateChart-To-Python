...
class Parser:
    ...
    def puml_to_ast(self):
        ...
        self.tree.create_node(
            f"transitions_in_{self.parent}",
            f"transitions_in_{self.parent}",
            parent=self.parent,
        )
        self.tree.create_node(
            f"states_in_{self.parent}", f"states_in_{self.parent}", parent=self.parent
        )
            ...
            if len(data[line]) >= i + 2:
                ...
                if data[line][i + 1] == "-->" or data[line][i + 1] == "->":
                    self.create_transition(
                        data[line], i, "_".join(parent.split("_")[:1])
                    )
    ...
    def create_transition(self, line, i, parent):
        ...
        if (
            self.tree.get_node(f"{source_state}_in_{parent}") is None
            and source_state != "[*]"
        ):
            self.tree.create_node(
                source_state, f"{source_state}_in_{parent}", f"states_in_{parent}"
            )
        ...
        if len(history_check) == 2:
            ...
            self.tree.create_node(
                f"history_state_{history_check[0]}", random.random(), g_id
            )

            if self.tree.get_node(f"h_{history_check[0]}") is None:
                self.tree.create_node(
                    f"history",
                    f"h_{history_check[0]}",
                    parent=f"{history_check[0]}_in_{parent}",
                )
            ...
            if (
                self.tree.get_node(f"{goal_state}_in_{parent}") is None
                and source_state != "[*]"
            ):
                self.tree.create_node(
                    goal_state, f"{goal_state}_in_{parent}", f"states_in_{parent}"
                )
    ...
    def create_state(self, data, line, i):
        ...
        if (
            self.tree.get_node(f"{new_node}_in_{self.parent}") is None
            and new_node != "[*]"
        ):
            self.tree.create_node(
                new_node, new_node_id, parent=f"states_in_{self.parent}"
            )  # create state node

            self.tree.create_node(
                f"transitions_in_{new_node}",
                f"transitions_in_{new_node}",
                parent=new_node_id,
            )  # create Transitions leaf
            self.tree.create_node(
                f"states_in_{new_node}", f"states_in_{new_node}", parent=new_node_id
            )  # create States leaf
        ...
        if len(history_check) == 2:
            self.tree.create_node(
                f"history_state_{new_node}",
                f"h_{new_node}",
                parent=f"states_in_{new_node}",
            )
    ...
    def create_entry(self, line, i, parent):
        ...
        self.tree.create_node(
            entry_clear, f"{entry_name}_{entry_underline}", entry_name
        )
...
if __name__ == "__main__":
    parser = Parser(warnings=False, file="../examples/inputs/coffeeMachine.puml")
    parser.puml_to_ast()
    parser.tree.show()
